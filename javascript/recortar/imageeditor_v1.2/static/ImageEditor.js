...
ImageEditor.processImage = function(args){
	...
	request.open("POST", "/processImage", true);
...
ImageEditor.loadImage = function(){
	ImageEditor.loaderImage.setAttribute("src", "/getImage?t=" + (new Date).getTime());
};
...
ImageEditor.im_feeling_lucky = function(){
    ImageEditor.processImage("action=im_feeling_lucky");
};
ImageEditor.crop = function(){
    ...
    left_x = left / ImageEditor.w;
    top_y = top / ImageEditor.h;
    right_x = (left + width) / ImageEditor.w;
    bottom_y = (top + height) / ImageEditor.h;
